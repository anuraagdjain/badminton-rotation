...
export function GameManager({ initialParticipants, initialCourtCount }: GameManagerProps) {
  ...
  useEffect(() => {
    if (initialParticipants && initialCourtCount) {
      const state = startGame(initialParticipants, initialCourtCount);
      setGameState(state);
      saveGame(state);
      router.replace("/");
    } else {
    ...
    setIsLoaded(true);
  }, [initialParticipants, initialCourtCount, router]);
